...
double MinSnapTraj::EndTime() { return times_.sum(); }
...
void MinSnapTraj::GenerateConstraints(
    ...
    Eigen::VectorXd& b_yaw)
{
    ...
    std::cout << "time powers" << std::endl;
    std::cout << time_powers << std::endl;
    ...
    for (int joint = 0; joint < num_internal_joints_; ++joint)
    {
        ...
        for (int derivative = 1; derivative <= kPosMinDerivative; ++derivative)
        {
            ...
            DenseA(constraint_count, kCoeffCount * (joint + 1) + derivative) =
